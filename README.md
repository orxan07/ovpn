...
`app deploy` дополнительно синхронизирует уже применённые whitelist-пресеты:
если в коде пресета появились новые домены/IP-диапазоны, они будут добавлены в
`/etc/sing-box/config.json` и `sing-box` перезапустится один раз. Новые пресеты
сами по себе не включаются — их всё ещё нужно применять вручную в панели.
Для совместимости со старой `/usr/local/bin/app` такая же синхронизация запускается
после `npm install` на VPS.

Bootstrap/restore также приводят окружение в рабочее состояние после переезда:
обновляют sudoers для пользователя панели, чинят внешний интерфейс в `wg0.conf`,
ставят зависимости для QR-кодов и при необходимости включают fallback-wrapper для
`wg-quick@wg0`.
